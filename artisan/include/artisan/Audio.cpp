...
audio_context* initialize_audio_context(artisan::Window applet, unsigned play_freq_hz, int buffered_samples, int playing_pool_count)
{
#ifdef _WIN32
    void* win = glfwGetWin32Window(applet.m_Window);
#elif __linux__
#ifndef MUZZLE_FORCE_WAYLAND
    void* win = glfwGetX11Window(applet.m_Window);
#else
    void* win = glfwGetWaylandWindow(applet.m_Window);
#endif
#elif __APPLE__
    void* win = glfwGetCocoaWindow(applet.m_Window);
#endif
...
{
    if (cs_insert_sound(ctx, &sound->sound) == 0) std::cerr << "Some  error audioo!!!!!! sorry bout that  :((((((((\n";
}
